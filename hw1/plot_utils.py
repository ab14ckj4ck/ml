...
def plot_scatterplot_and_polynomial(x: np.ndarray, 
    ...
    f_theta = f"{theta[0]:.3f}"
    for i in range(1, len(theta)):
        f_theta += f" + {theta[i]:.3f}·x^{i}"


    f_theta = f_theta[:-1]


    K = len(theta) - 1 # Theta = K + 1
    x_plot = np.linspace(np.min(x), np.max(x), 500)
    Design_Matrix_plot = compute_polynomial_design_matrix(x_plot, K)
    y_plot = Design_Matrix_plot @ theta

    plt.scatter(x, y)
    plt.plot(x_plot, y_plot, label=f"fθ(x) = {f_theta}", color="red")
...
def plot_scatterplot_and_line(x: np.ndarray, 
    ...
    reg_line = theta[0] + theta[1] * x

    plt.scatter(x, y, color="blue")
    plt.plot(x, reg_line, label=f"fθ(x) = {theta[0]:.2f} + {theta[1]:.2f}·x", color="red")
