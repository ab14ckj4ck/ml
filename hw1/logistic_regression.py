...
def create_design_matrix_dataset_1(X_data: np.ndarray) -> np.ndarray:
    ...
    X = np.column_stack([x1, x2, x1 * x2, x1 ** 2, x2 ** 2, (x1 - 10) * (x2 - 20)])
...
def create_design_matrix_dataset_3(X_data: np.ndarray) -> np.ndarray:
    ...
    x1 = X_data[:, 0]
    x2 = X_data[:, 1]
    X = np.column_stack([x1, x2, x1**2, x1**3, x1**4, x1**5, x1**6, x1*x2, x1**2 * x2, x1 * x2**2])

    '''Best Parameters for dataset 3:
            'penalty' : 'l1',
            'solver' : 'liblinear',
            'C' : 1.0,'''
