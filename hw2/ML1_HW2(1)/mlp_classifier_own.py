...
class MLPClassifierOwn():
    ...
    def binary_cross_entropy_loss(y_true: int, prob: Scalar) -> Scalar:
        ...
        y_prop_log = y_true * prob.log()

        y_neg_log = (1 - y_true) * (1 - prob).log()
        bce = -(y_prop_log + y_neg_log)
        return bce
    ...
    def l2_regularization_term(self) -> Scalar:
        ...
        omega = (self.alpha / (2 * abs(self.batch_size))) * sum(p * p for p in self.model.parameters())
        return omega
