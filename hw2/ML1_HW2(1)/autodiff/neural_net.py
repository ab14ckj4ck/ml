...
class FeedForwardLayer(Module):
    def __init__(self, num_inputs: int, num_outputs: int, use_relu: bool):
        ...
        for _ in range(num_outputs):
            neuron = Neuron(num_inputs, use_relu)
...
class MultiLayerPerceptron(Module):
    def __init__(self, num_inputs: int, num_hidden: List[int], num_outputs: int):
        ...
        for l in range(len(num_hidden) + 1):
            if l == 0:
                layer = FeedForwardLayer(num_inputs, num_hidden[l], True)
            elif l < len(num_hidden):
                layer = FeedForwardLayer(num_hidden[l - 1], num_hidden[l], True)
            else:
                layer = FeedForwardLayer(num_hidden[l - 1], num_outputs, False)
